...
impl NetworkAdapters {
    /// Check whether any adapter's MAC address has the locally administered
    /// bit set in every octet — a pattern produced by certain cheat tools
    /// that generate virtual network adapters.
    pub fn has_maple_pattern(&self) -> bool {
        self.adapters
            .split('.')
