...
pub async fn remove_spectating(ctx: &RequestContext, session_id: Uuid) -> anyhow::Result<()> {
    let mut redis = ctx.redis.get().await?;
...
pub async fn remove_members(ctx: &RequestContext, host_session_id: Uuid) -> anyhow::Result<()> {
    let mut redis = ctx.redis.get().await?;
    let key = make_key(host_session_id);
